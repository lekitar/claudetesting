...
export default function ReviewFormClient({ user, cycle, review, subject, sections, answersMap: initialAnswers, selfAnswers }) {
  ...
  const [showSelfRef, setShowSelfRef] = useState(false);
  ...
  const firstName = subject.full_name?.split(' ')[0] || subject.full_name || 'esta persona';
  const initials = subject.full_name ? subject.full_name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : '??';

  const saveAnswer = useCallback(debounce(async (questionId, type, value) => {
    setSaveState('saving');
    await supabase.from('review_answers').upsert({
    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF', fontFamily: 'system-ui, -apple-system, sans-serif', WebkitFontSmoothing: 'antialiased' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ fontSize: 13.5, fontWeight: 600, color: '#0C0A09' }}>Revisión de desempeño</span>
          <span style={{ fontSize: 13, color: '#A8A29E' }}>{cycle.name}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {selfAnswers.length > 0 && (
            <button onClick={() => setShowSelfRef(!showSelfRef)}
              style={{ fontSize: 12.5, fontWeight: 500, color: showSelfRef ? '#0C0A09' : '#78716C', background: showSelfRef ? '#F5F5F4' : 'none', border: '1px solid #E7E5E4', borderRadius: 6, padding: '5px 10px', cursor: 'pointer' }}
            >
              {showSelfRef ? 'Ocultar' : 'Ver'} autoevaluación de {firstName}
            </button>
          )}
        ...
        <div style={{ width: 240, flexShrink: 0, paddingTop: 40, paddingRight: 40 }}>
          <div style={{ position: 'sticky', top: 76 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              style={{ background: '#FAFAF9', border: '1px solid #E7E5E4', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#1C1917', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, color: '#78716C', flexShrink: 0 }}>
                  {initials}
                </div>
                <div>
                  <p style={{ margin: 0, fontSize: 13.5, fontWeight: 600, color: '#0C0A09' }}>{subject.full_name}</p>
                  <p style={{ margin: '1px 0 0', fontSize: 12, color: '#A8A29E' }}>{subject.job_title || '—'}</p>
                </div>
              </div>
              {subject.department && (
                <span style={{ fontSize: 11, color: '#A8A29E', background: '#FFFFFF', border: '1px solid #E7E5E4', borderRadius: 6, padding: '2px 7px' }}>{subject.department}</span>
              )}
            </motion.div>
                ...
                return (
                  <motion.button key={s.id}
                    ...
                    style={{
                      ...
                      borderRadius: 8, border: 'none', background: isActive ? '#F5F5F4' : 'transparent',
        ...
        <div style={{ flex: 1, paddingTop: 40, paddingBottom: 80, maxWidth: 660 }}>
          <AnimatePresence mode="wait">
            ...
            >
              <div style={{ marginBottom: 36 }}>
                ...
                {section?.description && (
                  <p style={{ margin: 0, fontSize: 15, color: '#78716C', lineHeight: 1.65 }}>
                    {section.description.replace(/\{name\}/g, firstName)}
                  </p>
                )}
              </div>
              ...
              <AnimatePresence>
                {showSelfRef && selfAnswers.length > 0 && (
                  <motion.div
                    initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                    transition={{ duration: 0.2 }}
                    style={{ overflow: 'hidden', marginBottom: 28 }}
                  >
                    <div style={{ background: '#FAFAF9', border: '1px solid #E7E5E4', borderRadius: 10, padding: '16px 18px' }}>
                      <p style={{ margin: '0 0 12px', fontSize: 12, fontWeight: 600, color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        Autoevaluación de {firstName}
                      </p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {selfAnswers.filter(a => a.answer_text || a.answer_rating).slice(0, 3).map(a => (
                          <div key={a.id}>
                            <p style={{ margin: '0 0 3px', fontSize: 12, color: '#78716C', fontWeight: 500 }}>{a.template_questions?.label}</p>
                            {a.answer_text && <p style={{ margin: 0, fontSize: 13, color: '#292524', lineHeight: 1.5 }}>{a.answer_text}</p>}
                            {a.answer_rating && <p style={{ margin: 0, fontSize: 13, color: '#292524' }}>Valoración: {a.answer_rating}/5 — {RATING_LABELS[a.answer_rating - 1]}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 52, paddingTop: 28, borderTop: '1px solid #F5F5F4' }}>
                <motion.button whileTap={{ scale: 0.97 }}
                  ...
                  style={{ padding: '10px 20px', borderRadius: 8, border: '1.5px solid #E7E5E4', background: '#FFFFFF', fontSize: 14, fontWeight: 500, cursor: currentSection === 0 ? 'not-allowed' : 'pointer', color: currentSection === 0 ? '#D6D3D1' : '#0C0A09' }}
